...
public class SemanticAnalyzer implements SemanticVisitor {
  ...
  public void visit(FunctionDefinitionNode node) {
    ...
    currentScope = new FunctionScope(currentScope, symbol);
      if (node.getFormalParameters() != null) {
          node.getFormalParameters().accept(this);
      }
  ...
  public void visit(ReturnStatementNode node) {
    ...
    if (!(ExpressionType.getType(funcType) == expressionType) && (funcIsArray
        == assignableIsArray)) {
      throw new SemanticException("Expected return type: " + funcType + " Got: "
          + ExpressionType.values()[expressionType].name().toLowerCase(), node.getToken());
    }
  }
  ...
  public void visit(FunctionCall node) {
    ...
    for (int i = 0, size = actualParams.size(); i < size; i++) {
      if (!(funcParams.get(i) instanceof ArrayType == actualParams.get(i).isArray() &&
        ...
        throw new SemanticException("Parameter " + (i + 1) + " type doesn't match",
            actualParams.get(i).getToken());
      }
    }
  }
  ...
  public void visit(ArrayLengthNode node) {
    ...
    variable.accept(this);
      if (!variable.isArray()) {
          throw new SemanticException("length accepts only array type variables!",
              variable.getToken());
      }
    node.setType(ExpressionType.INT.ordinal());
  }
  ...
  public void visit(VariableNode node) {
    ...
    if (node.getIndex() != null) {
        if (!isArrayType) {
            throw new SemanticException(varSymbol.getName() + " is not array!", node.getToken());
        }
      node.getIndex().accept(this);
      if (node.getIndex().getType() != ExpressionType.INT.ordinal()) {
        throw new SemanticException("Index of array variable must be of type int!",
            node.getIndex().getToken());
      }
    }
